...
[originalSignal, fs] = audioread('Cat/cat_sample.wav');
...
for n = 1:BITS;
  ...
  print(["Cat/Quantized_signal ", int2str(n), ".png"], "-dpng", "-color");
  
  roundedSignal = round(originalSignal*x)/x;
  
  ##Audio save function from folder
  audiowrite(['Cat/cat', int2str(n), '.wav'], roundedSignal, fs);
end
...
file1 = ['Cat/cat' , int2str(BITS), '.wav'];
file2 = ['Cat/cat' , int2str(BITS + 1), '.wav'];
...
print(['Cat/Fig. 1 - Original Plots.png'], "-dpng", '-S1600,900');   

#############################################################################

##Designing FIR Filter
cutOff = [750 3300]/(fs/2);
...
print(['Cat/Fig. 2 - Filtered Plots.png'],"-dpng", '-S1600,900'); 

audiowrite(['Cat/cat_filtered.wav'], fqFiltered, fp);