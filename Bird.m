...
[originalSignal, fs] = audioread('Bird/bird_sample.wav');
...
for n = 1:BITS;
  ...
  print(["Bird/Quantized_signal ", int2str(n), ".png"], "-dpng", "-color");
  
  roundedSignal = round(originalSignal*x)/x;
  
  ##Audio save function from folder
  audiowrite(['Bird/bird', int2str(n), '.wav'], roundedSignal, fs);
end
...
file1 = ['Bird/bird' , int2str(BITS), '.wav'];
file2 = ['Bird/bird' , int2str(BITS + 1), '.wav'];
...
print(['Bird/Fig. 1 - Original Plots.png'], "-dpng", '-S1600,900');   

#############################################################################

##Designing FIR Filter
cutOff = [3000 5200]/(fs/2);
...
print(['Bird/Fig. 2 - Filtered Plots.png'],"-dpng", '-S1600,900'); 

audiowrite(['Bird/bird_filtered.wav'], fqFiltered, fp);