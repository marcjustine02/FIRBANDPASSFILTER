...
[originalSignal, fs] = audioread('Chicken/chicken_sample.wav');
...
for n = 1:BITS;
  ...
  print(["Chicken/Quantized_signal ", int2str(n), ".png"], "-dpng", "-color");
  
  roundedSignal = round(originalSignal*x)/x;
  
  ##Audio save function from folder
  audiowrite(['Chicken/chicken', int2str(n), '.wav'], roundedSignal, fs);
end
...
file1 = ['Chicken/chicken' , int2str(BITS), '.wav'];
file2 = ['Chicken/chicken' , int2str(BITS + 1), '.wav'];
...
print(['Chicken/Fig. 1 - Original Plots.png'], "-dpng", '-S1600,900');   

#############################################################################

##Designing FIR Filter
cutOff = [500 3500]/(fs/2);
...
print(['Chicken/Fig. 2 - Filtered Plots.png'],"-dpng", '-S1600,900'); 

audiowrite(['Chicken/chicken_filtered.wav'], fqFiltered, fp);