...
[originalSignal, fs] = audioread('Dog/dog_sample.wav');
...
for n = 1:BITS;
  ...
  print(["Dog/Quantized_signal ", int2str(n), ".png"], "-dpng", "-color");
  
  roundedSignal = round(originalSignal*x)/x;
  
  ##Audio save function from folder
  audiowrite(['Dog/dog', int2str(n), '.wav'], roundedSignal, fs);
end
...
file1 = ['Dog/dog' , int2str(BITS), '.wav'];
file2 = ['Dog/dog' , int2str(BITS + 1), '.wav'];
...
print(['Dog/Fig. 1 - Original Plots.png'], "-dpng", '-S1600,900');   

#############################################################################

##Designing FIR Filter
cutOff = [700 1500]/(fs/2);
...
print(['Dog/Fig. 2 - Filtered Plots.png'],"-dpng", '-S1600,900'); 

audiowrite(['Dog/dog_filtered.wav'], fqFiltered, fp);